...
async def auth_register():
    try:
        session[
            "authSession"
        ]  # Will raise KeyError if no session is active, meaning the line below will not get executed.
        redirect_url = request.referrer
        if request.referrer is None:
            redirect_url = "/"
        return redirect(
            redirect_url, Response=Response("Already logged in", status=400)
        )
    except KeyError:
        ...
        db_Client["fvWeb"]["Accounts"].insert_one(
            {
                "uuid": UUID,
                "email": email,
                "username": username,
...
async def auth_login():
    try:
        ...
        if request.referrer is None:
            redirect_url = "/"
        return redirect(
            redirect_url, Response=Response("Already logged in", status=200)
        )
    except KeyError:
        username = request.form["username"]
        password = hashlib.sha256(request.form["password"].encode("utf-8")).hexdigest()
